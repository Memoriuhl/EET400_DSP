...
#define lds 0x3   /* Assumes bits 0-1 of GPIO0 is connected to LEDs  */
...
/*short int*/float fir_coeff[numcoeff]={//coefficients signed fixed points
		0.0800943102542603,
		0.339811379491480,
		0.339811379491480,
		0.339811379491480,
...
int main()
{
   ...
   short int j[numcoeff]={2625,  11135,  11135,  11135,   2625
};
  ...
  //  XGpio_DiscreteWrite(&Gpio0, CH1, led);//write switches to leds
    ...
    i=0;
    XGpio_DiscreteWrite(&Gpio2, CH2, 12500);
  ...
  while(1){/*
	      ...
	      {
	         case 0:
	        	XGpio_DiscreteWrite(&Gpio2, CH2, 6250); //counter at 16KHz
	        	break;
	         case 1:
	        	XGpio_DiscreteWrite(&Gpio2, CH2, 4535);//counter at 44.110KHz
	            break;
	         case 2:
...
//	         	break;
	         default:
	        	XGpio_DiscreteWrite(&Gpio2, CH2, 6250);//default counter at 8Khz
...
/********************Filter Calculations*******************************/
		...
		for(i=0;i<numcoeff;i++){
			fir_mac=fir_mac+fir_coeff[i]*(int)j[i];
		}
		fir_out = (fir_mac>>15)+2047;//shifting 15 bits

		/********************End Filter Calculations*******************************/
       //adjust binary point to convert back from fixed point with offset for DA2
       fir_out = DtoAData+2047;//should shift 15 places for unity gain
       data_out = DtoAData;
